var NoteToSelf = {
    ...
    addNote: function(noteObj){
        if(this.editing !== true)
        {
            ...
            var noteButtons = document.createElement('ul');
    ...
    editNote: function (e) {
        var edit = e.target;
        var save = e.target.previousSibling;
        var para = e.target.parentNode.previousSibling;
        var form = document.createElement('textarea');
        form.setAttribute('id','noteText');
    ...
    createNote: function (noteNode) {
        ...
        var noteId = noteNode.getAttribute('data-id');
        var txt = document.getElementById('noteText');
        var save = noteNode.childNodes[1].childNodes[0];
        var edit = save.nextSibling;
        
        //Textarea empty?
        if(txt.value !== '')
        {
            //Create JSON note object
            var newNote = {title: '', date: (new Date()).getTime(), note: txt.value};
    ...
    removeNote: function (el){
        ...
        var note = el.parentNode.parentNode;
        store.splice(id,1);
        //CSS animation
    ...
    initialize: function () {
        ...
        {
            this.addNote(store[i]);
        }
    },
    //Load notes from database via AJAX
    loadDB: function () {
        var request = new XMLHttpRequest();
        var loadDlg = document.getElementById('loadInfo');
    ...
    saveDB: function () {
        var request = new XMLHttpRequest();
        var saveDlg = document.getElementById('saveInfo');
        request.open('POST','php/user.php');
        request.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
